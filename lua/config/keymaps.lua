...
map("n", "<F7>", function()
	dapui().toggle()
end, opts("[DAP-UI] DAP UI 토글"))
map("n", "<F8>", function()
	dap().set_breakpoint(vim.fn.input("조건식: "))
end, opts("[DAP] 조건부 브레이크포인트 설정"))
map("n", "<F9>", function()
	dap().toggle_breakpoint()
end, opts("[DAP] 브레이크포인트 토글"))
map("n", "<F10>", function()
	dap().step_over()
...
map("n", "<leader>ada", "<cmd>ClaudeCodeDiffAccept<cr>", opts("[Claude] 변경사항 수락"))
map("n", "<leader>adr", "<cmd>ClaudeCodeDiffDeny<cr>", opts("[Claude] 변경사항 거절"))
