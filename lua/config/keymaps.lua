...
map("n", "<leader>er", "<cmd>Neotree reveal<cr>", opts("[Neo-tree] 현재 파일 탐색기에서 표시"))
map("n", "<leader>ge", "<cmd>Neotree git_status toggle<cr>", opts("[Neo-tree] Git 상태 탐색기 토글"))
...
map("n", "]t", function() todo_comments().jump_next() end, opts("[Todo-comments] 다음 TODO로 이동"))
map("n", "[t", function() todo_comments().jump_prev() end, opts("[Todo-comments] 이전 TODO로 이동"))
...
map("n", "]h", function() gitsigns().next_hunk() end, opts("[Git Gitsigns] 다음 hunk로 이동"))
map("n", "[h", function() gitsigns().prev_hunk() end, opts("[Git Gitsigns] 이전 hunk로 이동"))
map("n", "<leader>gp", function() gitsigns().preview_hunk() end, opts("[Git Gitsigns] hunk 미리보기"))
map("n", "<leader>gb", function() gitsigns().toggle_current_line_blame() end, opts("[Git Gitsigns] blame 토글"))

-- nvim-ufo (코드 폴딩)
map("n", "zR", function() ufo().openAllFolds() end, opts("[UFO] 모든 fold 열기"))
map("n", "zM", function() ufo().closeAllFolds() end, opts("[UFO] 모든 fold 닫기"))
map("n", "<leader>zp", function() ufo().peekFoldedLinesUnderCursor() end, opts("[UFO] fold 내용 미리보기"))
...
map("n", "<F5>",      function() dap().continue() end,           opts("[DAP] 디버깅 시작/계속"))
map("n", "<F9>",      function() dap().toggle_breakpoint() end,  opts("[DAP] 브레이크포인트 토글"))
map("n", "<F10>",     function() dap().step_over() end,          opts("[DAP] Step Over"))
map("n", "<F11>",     function() dap().step_into() end,          opts("[DAP] Step Into"))
map("n", "<S-F11>",   function() dap().step_out() end,           opts("[DAP] Step Out"))
map("n", "<S-F5>",    function() dap().terminate() end,          opts("[DAP] 디버깅 종료"))
map("n", "<F7>",      function() dapui().toggle() end,           opts("[DAP-UI] DAP UI 토글"))
map("n", "<F8>",      function() dap().terminate(); dapui().close() end, opts("[DAP] 디버거 종료 + UI 닫기"))
map("n", "<leader>dt", function() dap_go().debug_test() end,     opts("[DAP-Go] Go 테스트 디버깅"))
