...
**카테고리:** `lua/plugins/debug/` (기존 dap.lua 옆 또는 통합)

- 이미 `nvim-dap` + `nvim-dap-ui` 가 Go 용으로 설치돼 있어 Python 어댑터만 추가하면 됨
- pytest / unittest 함수 단위 디버깅 (`<leader>dn` 등으로 현재 테스트 함수 실행)
- `debugpy` 자동 설치 가능 (Mason)

**왜 필요한가:** 현재 Go 만 디버거가 있고 Python 은 `print` 디버깅에 의존하게 됩니다.
...
| 2 | nvim-dap-python | Python 디버깅 가능 |
