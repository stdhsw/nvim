...
| 단축키     | 설명                                             |
|------------|--------------------------------------------------|
| `<Ctrl-n>`    | `[멀티커서]` 현재 단어 선택 / 다음 매칭 추가 선택  |
| `<Option-Down>` | `[멀티커서]` 아래로 커서 추가 (Option+↓)         |
| `<Option-Up>`   | `[멀티커서]` 위로 커서 추가 (Option+↑)           |
| `<Ctrl-Shift-n>` | `[멀티커서]` 현재 단어의 모든 매칭에 커서 생성 |
| `\\/`      | `[멀티커서]` 정규식 검색 후 커서 배치              |
...
**사용 예시: 변수명 일괄 변경**
1. 변경할 단어 위에 커서를 놓고 `<Ctrl-n>`
2. `<Ctrl-n>` 반복으로 변경할 위치를 추가 선택 (건너뛰려면 `q`)
3. `c` 로 변경 모드 진입 → 새 텍스트 입력 → `<Esc>`

**사용 예시: 여러 줄의 임의 위치에 커서 배치**
1. 원하는 위치에서 `<Option-Down>` / `<Option-Up>` 으로 커서 추가
...
| 단축키            | 설명                                  |
|-------------------|---------------------------------------|
| `<Ctrl-n>` / `<Ctrl-p>` | `[검색]` 다음 / 이전 결과로 이동       |
| `<Ctrl-x>`           | `[검색]` 수평 분할로 파일 열기         |
| `<Ctrl-v>`           | `[검색]` 수직 분할로 파일 열기         |
| `<Ctrl-t>`           | `[검색]` 새 탭으로 파일 열기           |
| `<Ctrl-u>` / `<Ctrl-d>` | `[검색]` 미리보기 창 스크롤            |
| `<Ctrl-q>`           | `[검색]` 결과를 quickfix 목록으로 전송 |
| `<Tab>`           | `[검색]` 다중 선택 토글                |
| `<Esc>`           | `[검색]` telescope 닫기                |
...
| `l`    | `[Git Log]` git log 메뉴 열기 (서브 키 입력 대기) |

**neogit log 서브메뉴 (`l` 진입 후)**

| 단축키 | 설명                                          |
|--------|-----------------------------------------------|
| `ll`   | `[Git Log]` 현재 브랜치 git 커밋 로그 보기     |
| `la`   | `[Git Log]` 모든 브랜치 git 로그 (tree 그래프) |
| `lh`   | `[Git Log]` HEAD 기준 git 커밋 로그 보기       |

---

## [git] sindrets/diffview.nvim

키워드: git diff 비교 변경사항 머지 충돌 merge conflict 좌우분할 로그 히스토리 history log

파일 변경사항을 좌우 분할 화면으로 비교하고, 커밋 히스토리를 탐색한다.

| 단축키       | 설명                                       |
|--------------|--------------------------------------------|
| `<leader>gv` | `[Git Diff]` git diffview 열기              |
| `<leader>gV` | `[Git Diff]` git diffview 닫기              |
| `<leader>gh` | `[Git Log]` 현재 파일의 git 커밋 히스토리    |
| `<leader>gH` | `[Git Log]` 브랜치 전체 git 커밋 히스토리    |

| 명령어                   | 설명                                      |
|--------------------------|-------------------------------------------|
| `:DiffviewOpen`          | `[Git Diff]` 현재 git 변경사항 diff 보기   |
| `:DiffviewOpen <rev>`    | `[Git Diff]` 특정 git 리비전과 비교         |
| `:DiffviewClose`         | `[Git Diff]` git diffview 닫기              |
| `:DiffviewFileHistory`   | `[Git Log]` 브랜치 전체 git 커밋 히스토리    |
| `:DiffviewFileHistory %` | `[Git Log]` 현재 파일의 git 커밋 히스토리    |

---

## [git] akinsho/git-conflict.nvim

키워드: git 충돌 conflict merge rebase 마커 marker 강조 highlight ours theirs

git merge/rebase 중 발생한 충돌 마커와 OURS/THEIRS 영역을 색상으로 강조한다.
이동은 기본 키맵으로, 해결은 명령어로 수행한다.

| 단축키       | 설명                                  |
|--------------|---------------------------------------|
| `]x` / `[x`  | `[Git Conflict]` 다음/이전 충돌로 이동 |

| 명령어                       | 설명                          |
|------------------------------|-------------------------------|
| `:GitConflictChooseOurs`     | OURS(현재 브랜치) 선택        |
| `:GitConflictChooseTheirs`   | THEIRS(들어오는 변경) 선택    |
| `:GitConflictChooseBoth`     | 양쪽 모두 선택                |
| `:GitConflictChooseNone`     | 양쪽 모두 제거                |
| `:GitConflictListQf`         | 충돌 quickfix 목록            |
...
|--------------|-----------------------------------------------|
| `gd`         | `[LSP이동]` 정의로 이동                         |
| `gD`         | `[LSP이동]` 정의를 옆 창에 표시 (없으면 vsplit) |
| `gi`         | `[LSP이동]` 구현으로 이동                       |
...
| 단축키    | 설명                                             |
|-----------|--------------------------------------------------|
| `<Ctrl-n>`   | `[자동완성]` 다음 항목 선택                        |
| `<Ctrl-p>`   | `[자동완성]` 이전 항목 선택                        |
| `<Ctrl-y>`   | `[자동완성]` 현재 항목 바로 확정                   |
| `<Ctrl-e>`   | `[자동완성]` 자동완성 창 닫기                      |
| `<CR>`    | `[자동완성]` 선택 항목 확정 (명시적 선택만)        |
| `<Tab>`   | `[자동완성]` 다음 항목 선택 / 스니펫 다음 위치     |
| `<Shift-Tab>` | `[자동완성]` 이전 항목 선택 / 스니펫 이전 위치     |
...
## [debug] mfussenegger/nvim-dap + nvim-dap-ui + nvim-dap-go + nvim-dap-python

키워드: 디버깅 디버거 브레이크포인트 중단점 step 변수확인 Go Python delve debugpy pytest debug breakpoint

DAP 기반 Go / Python 디버깅 환경. 디버깅 시작 시 UI 자동 열림, 종료 시 자동 닫힘.
`.env` 파일이 있으면 환경변수를 자동 로드한다 (Go 디버깅 시).

**사전 요구사항**
- Go: `brew install delve`
- Python: `debugpy` — `mason-tool-installer` 가 nvim 시작 시 자동 설치
...
**Python 전용 단축키 (nvim-dap-python)**

테스트 러너는 `pytest` 사용. 메서드/클래스 디버깅은 커서가 해당 블록 안에 있어야 동작한다.

| 단축키        | 모드   | 설명                                          |
|---------------|--------|-----------------------------------------------|
| `<leader>dpt` | Normal | `[디버깅]` 커서 위치의 Python 테스트 메서드 디버깅 |
| `<leader>dpc` | Normal | `[디버깅]` 커서 위치의 Python 테스트 클래스 디버깅 |
| `<leader>dps` | Visual | `[디버깅]` 선택 영역을 Python REPL 에서 평가      |
...
| 단축키       | 설명                                   |
|--------------|----------------------------------------|
| `<Shift-h>`      | `[버퍼]` 이전 버퍼로 이동               |
| `<Shift-l>`      | `[버퍼]` 다음 버퍼로 이동               |
| `<leader>w`  | `[버퍼]` 현재 버퍼 닫기                 |
| `<leader>W`  | `[버퍼]` 현재 버퍼 외 모두 닫기         |
...
키워드: 테마 색상 컬러 다크 라이트 theme color dark light github kanagawa nightfox vague one

5개 테마 플러그인. `<leader>tc` 로 실시간 미리보기하며 전환 가능.
...
| `:colorscheme kanagawa-wave`             | `[테마]` kanagawa wave (다크, 기본)  |
| `:colorscheme kanagawa-dragon`           | `[테마]` kanagawa dragon (더 어두운 다크) |
| `:colorscheme kanagawa-lotus`            | `[테마]` kanagawa lotus (라이트)    |
| `:colorscheme nightfox`                  | `[테마]` nightfox (다크, 기본)      |
| `:colorscheme duskfox`                   | `[테마]` nightfox duskfox (자줏빛 다크) |
| `:colorscheme nordfox`                   | `[테마]` nightfox nordfox (nord 감성 다크) |
| `:colorscheme terafox`                   | `[테마]` nightfox terafox (청록 톤 다크) |
| `:colorscheme carbonfox`                 | `[테마]` nightfox carbonfox (가장 어두운) |
| `:colorscheme dayfox`                    | `[테마]` nightfox dayfox (라이트)   |
| `:colorscheme dawnfox`                   | `[테마]` nightfox dawnfox (파스텔 라이트) |
| `:colorscheme vague`                     | `[테마]` vague (뮤트한 파스텔 톤 다크) |
| `:colorscheme one`                       | `[테마]` vim-one (다크/라이트 자동) |
