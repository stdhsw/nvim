...
## [git] lazygit (toggleterm 연동)

키워드: git tui lazygit 터미널 gui

Git TUI 클라이언트. toggleterm의 float 터미널로 실행된다.

**사전 요구사항**: `brew install lazygit`

| 단축키       | 설명                     |
|--------------|--------------------------|
| `<leader>gg` | `[GitTUI]` lazygit 열기    |
...
키워드: 테마 색상 컬러 다크 라이트 theme color dark light catppuccin github kanagawa nightfox

4개 테마 플러그인. `<leader>tc` 로 실시간 미리보기하며 전환 가능.
...
| `:colorscheme catppuccin-mocha`          | `[테마]` catppuccin mocha (다크)    |
...
| `:colorscheme catppuccin-latte`          | `[테마]` catppuccin latte (라이트)  |
